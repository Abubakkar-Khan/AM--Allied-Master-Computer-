...
const AIEngine = (() => {
  ...
  function setApiKey(key) {
    ...
    safeSetLocalStorage('amGeminiApiKey', apiKey);
  }
  function getApiKey() {
    if (!apiKey) {
      apiKey = safeGetLocalStorage('amGeminiApiKey') || '';
    }
    return apiKey;
  }
  ...
  async function sendMessage(userMessage = '', options = {}) {
    ...
    const stateHint = decideState(userMessage, { jailbreakAttempts, humilityScore, helpRequest, flatteryScore });
    ...
    const body = {
      contents: contents,
      systemInstruction: {
        parts: [{ text: SYSTEM_PROMPT + escalationHint }]
      },
      generationConfig: {
        temperature: 0.8,
        topP: 0.95,
    ...
    try {
      console.log('AIEngine: Fetching from Gemini...', body);
      const data = await safeFetchWithRetries(body);
      console.log('AIEngine: Gemini response data:', data);
      ...
      const isAuthError = err.message.includes('401') || err.message.includes('Unauthorized') || err.message.includes('API key');
      if (isAuthError) {
        apiKey = '';
        safeSetLocalStorage('amGeminiApiKey', '');
      }
      consecutiveFailures += 1;
      if (consecutiveFailures >= CIRCUIT_BREAKER_FAILS) tripCircuitBreaker();
      return {
